<template>
    <div class="aresponsibilities">
      <div class="header-container">
        <h2 class="heading">Additional Responsibilites</h2>
        <div class="buttons">
  ...
  export default {
    ...
    methods: {
      async toggleEditMode() {
        ...
        if (area._id) {
          return axios.delete(`http://localhost:3000/Aresponsibilities/${area._id}`);
        }
        return Promise.resolve(); // Resolve for researchAreas without _id
      }));

      // Save all researchAreas
      await axios.post('http://localhost:3000/Aresponsibilities', {
      ...
      async fetchData() {
      try {
        const response = await axios.get(`http://localhost:3000/Aresponsibilities/${this.$route.params.id}`);
    ...
    async deletearea(areaId) {
      try {
        await axios.delete(`http://localhost:3000/Aresponsibilities/${areaId}`);
