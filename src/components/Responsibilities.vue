  ...
  if (!this.editMode) {
    try {
      ...
      await Promise.all(this.experiences.map(experience => {
        if (experience._id) {
          return axios.delete(`http://localhost:3000/Responsibilities/${experience._id}`);
        }
        return Promise.resolve(); // Resolve for experiences without _id
      }));

      // Save all experiences
      await axios.post('http://localhost:3000/Responsibilities', {
...
async fetchData() {
  try {
    const response = await axios.get(`http://localhost:3000/Responsibilities/${this.$route.params.id}`);
    ...
    async deleteDetail(detailId) {
      try {
        await axios.delete(`http://localhost:3000/Responsibilities/${detailId}`);
