  ...
  export default {
    ...
    methods: {
      async toggleEditMode() {
        ...
        if (area._id) {
          return axios.delete(`http://localhost:3000/Training/${area._id}`);
        }
      ...
      async fetchData() {
      try {
        const response = await axios.get(`http://localhost:3000/Training/${this.$route.params.id}`);
    ...
    async deletearea(areaId) {
      try {
        await axios.delete(`http://localhost:3000/Training/${areaId}`);
